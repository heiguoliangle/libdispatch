 ...
 * of threads fully managed by the system. No guarantee(保证) is made regarding
 ...
 *
 * @abstract
 * The prototype(原型) of blocks submitted to dispatch queues, which take no
 ...
 *
 * @discussion
 * The dispatch_async() function is the fundamental(基本的,根本的) mechanism(机制,原理) for submitting
 ...
 *
 * @discussion
 * Submits a block to a dispatch queue like predispatch_async(), however
 * dispatch_sync() will not return until the block has finished.
 *
 * Calls to dispatch_sync() targeting the current queue will result
 * in dead-lock. Use of dispatch_sync() is also subject to(受制于) the same
 ...
 *
 * @param iterations
 * The number of iterations(迭代) to perform.
 *
 * @param queue
 * The target dispatch queue to which the block is submitted.
 * The result of passing NULL in this parameter is undefined.
 ...
 * The code must not make any assumptions(假定) about the queue returned, unless it
 ...
 *
 * @abstract
 * Returns a well-known(众所周知的) global concurrent(并发) queue of a given priority level.
 *
 * @discussion

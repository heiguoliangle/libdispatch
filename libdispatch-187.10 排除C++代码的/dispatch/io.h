 ...
 * desired(要求) frequency of I/O handlers for long-running operations.
 *
 ...
 * Convenience wrappers(包装,装饰) around the dispatch I/O channel API, with simpler
 * callback handler semantics(语义) and no explicit(明确的) management of channel objects.
 * File descriptors passed to the convenience API are treated(治疗,作用于,视为) as streams, and
 * scheduling multiple operations on one file descriptor via the convenience API
 * may incur(引发) more overhead than by using the dispatch I/O channel API directly.
 */
 ...
 *			submitted.
 * @param handler	The handler to enqueue(排队) when data is ready to be
 *			delivered.
 ...
 *				relinquishes(放弃) control over the file descriptor.
